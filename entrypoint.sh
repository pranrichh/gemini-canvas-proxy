...
START_PAGE="${START_PAGE:-https://gemini.google.com}"
...
# Start D-Bus session and GNOME Keyring daemon
if command -v dbus-launch >/dev/null 2>&1; then
    echo "[entrypoint] Starting D-Bus session daemon"
    eval "$(dbus-launch --sh-syntax)"
fi

# NOTE: gnome-keyring is intentionally NOT started here.
# Without a D-Bus Secret Service, Chromium falls back to its built-in
# static 'peanuts' encryption key (v10 prefix), which is consistent
# across restarts and allows cookie/session persistence.
...
# ── 6. Proxy ──────────────────────────────────────────────────────────────────
# The Python native host is spawned automatically by Chromium when the extension 
# connects via native messaging. We do not start it in the background here to 
# avoid port conflicts on 8765.
...
shutdown() {
    ...
    # Gracefully terminate Chromium first and wait for it to flush session data
    kill -TERM "$CHROMIUM_PID" 2>/dev/null || true
    sleep 5
    # Then shut down everything else
    kill -TERM "$PROXY_PID" "$WEBSOCKIFY_PID" "$X11VNC_PID" \
        "$OPENBOX_PID" "$XVFB_PID" 2>/dev/null || true
...
# Run in background so the SIGTERM trap can fire and close Chromium gracefully.
